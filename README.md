...
**Carry** — `s25-lr2ldf`
![carry combined](visualization/carry_combined.gif)
...
BlazePose 3D keypoint data, covering all 5 camera views (1.80 GB zipped). Download from [Google Drive](https://drive.google.com/file/d/1kFead_cqOQhWXmUuxC7emiEo0Fn1GDDw/view?usp=sharing).
...
- **File structure**: each file is a sequence of blocks — a header line `<camera>-<subject>-<task parameters>-<filename>.MP4;<activity class>` (`activity class`: `0`=push, `1`=pull, `2`=sit, `3`=stand, `4`=walk, `5`=lift, `6`=carry — note this header line omits the `push_`/`lift_`/etc. activity prefix that `data/metadata.txt` identifiers have; the numeric class is the only activity marker here) followed by one line per tracked frame, `<frame_index>;<66 comma-separated values>` (66 = 22 keypoints × 3 X/Y/Z coordinates, see below). A blank line separates blocks; lifting/carrying recordings contain multiple blocks per file (one per trial), matching the multi-trial identifiers described in [File Naming Convention](#file-naming-convention)
